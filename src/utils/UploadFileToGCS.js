...
const uploadFileToGCS = async (fileBuffer, originalname, mimetype) => {
    if (!fileBuffer || fileBuffer.length === 0) {
        console.error('UploadFileToGCS: fileBuffer is empty or undefined.'); // Log jika buffer kosong
        throw new Error('File buffer is empty, cannot upload to GCS.');
    }
...
const deleteFileFromGCS = async (filePath) => {
    ...
    try {
        ...
        console.error(`Gagal menghapus file ${filePath} dari GCS:`, err);
        throw err;
    }
};
